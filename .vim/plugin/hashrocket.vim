...
if $DEV_DIR == '' && expand('<sfile>') =~# '/dotmatrix/\.vim/plugin/hashrocket\.vim$'
  let $DEV_DIR = expand('<sfile>')[0 : -38]
endif
if $DEV_DIR == '' && filereadable(expand('~/.bashrc'))
  let $DEV_DIR = expand(matchstr("\n".join(readfile(expand('~/.bashrc')),"\n")."\n",'\n\%(export\)\=\s*DEV_DIR="\=\zs.\{-\}\ze"\=\n'))
endif
if $DEV_DIR == ''
  let $DEV_DIR = substitute(system("bash -i -c 'echo \"$DEV_DIR\"'"),'\n$','','')
endif
if $DEV_DIR == ''
  let $DEV_DIR = expand('~/hashrocket')
endif

function! s:HComplete(A,L,P)
  let match = split(glob($DEV_DIR.'/'.a:A.'*'),"\n")
  return map(match,'v:val[strlen($DEV_DIR)+1 : -1]')
endfunction
command! -bar -nargs=1 Hcommand :command! -bar -bang -nargs=1 -complete=customlist,s:HComplete H<args> :<args><lt>bang> $DEV_DIR/<lt>args>
