if system("which rspec > /dev/null 2>&1") || File.exists?(".rspec")
  @spec = 'rspec'
else
  @spec = 'spec'
end
...
def run_spec(file)
  ...
  start_fresh "running specs for file #{file}..."
  options = " -f nested -c"
...
def run_test(type, file)
  return unless file.include?(" ") || File.exist?(file)
  start_fresh "running tests for file #{file}..."
  system("time rake test#{':' + type if type} TEST=#{file}")
end

def run_feature(file)
  return unless file.include?(" ") || File.exist?(file)
  start_fresh "running cucumber tests..."
  options = "-f pretty"
...
start_fresh("starting watchr") unless @watchr_started
@watchr_started = 'already running'
...
watch('^app/services/(.*)\.rb') {|md| run_spec("spec/services/#{md[1]}_spec.rb") }
watch('^app/subscriptions/(.*)\.rb') {|md| run_spec("spec/subscriptions/#{md[1]}_spec.rb") }
